...
void Demo_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Demo_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 21;}
...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 10],&lvar[3]); // line circom 32
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 10]); // line circom 38
...
// load src
Fr_sub(&expaux[0],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 9]); // line circom 98
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
ctx->componentMemory[coffset].templateName = "Demo";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&circuitConstants[2]); // line circom 14
