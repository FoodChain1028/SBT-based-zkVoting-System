...
// This contract is modified from SpartanLabs - zkSBT (https://github.com/SpartanLabsXyz/zk-sbt/blob/master/contracts/zkSBT.sol)
...
contract zkSBT is Ownable {
    ...
    mapping(address => ProofData) private souls;
    ...
    function mint(
        ...
        ProofData memory _soulData = ProofData(a, b, c, input);
     ...
     * However, projects can have it such that users can propose changes for the contract owner to update.
     */
    function updateSBT(address _soul, ProofData memory _soulData)
        public
        validAddress(_soul)
        returns (bool)
    {
     ...
     */
    function getPublicNum(address _soul)
        public
        view
        virtual
        validAddress(_soul)
     ...
     */
    function validateAttribute(address _soul, address verifierAddress, uint256[1] memory input)
        public
        view
        returns (bool)
    {
        ...
        ProofData memory _soulData = souls[_soul];
    ...
    function hasSoul(address _soul)
        ...
        returns (bool)
    {
        return souls[_soul].publicNum != 0;
    }
